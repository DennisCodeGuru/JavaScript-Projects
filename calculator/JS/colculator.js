...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed, we convert the the current number
    //displayed on the screen to a number and  then store the result in
    //Calculator.First_Operand if it doesn't alredy exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator alredy exists and if Wait second operand is true,
    ...
    if (First_Operand == null) {
    ...
    } else if (operator) {//shecks if an operator alredy exists
        ...
        //this will remove any trayling 0's
